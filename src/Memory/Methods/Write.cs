    ...
    public partial class Mem
    {
        ...
        public bool WriteMemory(string code, string type, string write, string file = "", System.Text.Encoding stringEncoding = null, bool RemoveWriteProtection = true)
        {
            ...
            else if (type.ToLower() == "string")
            {
                ...
                memory = memory.Concat(new byte[] {0}).ToArray();
                size = memory.Length;
            }
        ...
        /// <param name="file">path and name of ini file. (OPTIONAL)</param>
        public bool WriteBytes(string code, byte[] write, string file = "")
        {
            UIntPtr theCode;
            theCode = GetCode(code, file);
            return WriteProcessMemory(mProc.Handle, theCode, write, (UIntPtr)write.Length, IntPtr.Zero);
        }
