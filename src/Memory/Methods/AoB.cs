...
namespace Memory
{
    public partial class Mem
    {
        ...
        /// <returns>IEnumerable of all addresses found.</returns>
        public Task<IEnumerable<long>> AoBScan(ulong start, ulong end, string search, bool writable = false, bool executable = true, bool mapped = false, string file = "")
        {
        ...
        /// <param name="mapped">Include mapped addresses in scan</param>
        /// <returns>IEnumerable of all addresses found.</returns>
        public Task<IEnumerable<long>> AoBScan(ulong start, ulong end, string search, bool readable, bool writable, bool executable, bool mapped, string file = "")
        {
            return Task.Run(() =>
            {
                ...
                if (start < (ulong)proc_min_address.ToUInt64())
                    start = (ulong)proc_min_address.ToUInt64();

                if (end > (ulong)proc_max_address.ToUInt64())
                    end = (ulong)proc_max_address.ToUInt64();
        ...
        /// <param name="mapped">Include mapped addresses in scan</param>
        /// <returns>IEnumerable of all addresses found.</returns>
        public Task<IEnumerable<long>> AoBScan(ulong start, ulong end, byte[] search, byte[] mask, bool readable, bool writable, bool executable, bool mapped = false, string file = "")
        {
            return Task.Run(() =>
            {
                var memRegionList = new List<MemoryRegionResult>();
                ...
                Parallel.ForEach(memRegionList,
                                 (item, parallelLoopState, index) =>
                                 {
                                     long[] compareResults = CompareScan(item, search, mask);
        ...
        public Task<IEnumerable<long>> AoBScan(ulong start, ulong end, byte[] search, bool readable, bool writable, bool executable, bool mapped = false, string file = "")
        {
            return Task.Run(() =>
            {
                var memRegionList = new List<MemoryRegionResult>();
                ...
                Parallel.ForEach(memRegionList,
                                 (item, parallelLoopState, index) =>
                                 {
                                     long[] compareResults = CompareScan(item, search);
        ...
        public async Task<long> AoBScan(string code, ulong end, string search, string file = "")
        {
            ulong start = (ulong)GetCode(code, file).ToUInt64();

            return (await AoBScan(start, end, search, true, true, true, false, file)).FirstOrDefault();
        }
        private long[] CompareScan(MemoryRegionResult item, byte[] aobPattern)
        {
            ...
            try
            {
                buffer = Marshal.AllocHGlobal((int)item.RegionSize);
            }
            catch(Exception e) 
            {
                return new long[0];
            ...
            unsafe
            {
                do
                {
                    ...
                    result = FindPattern((byte*)buffer.ToPointer(), (int)bytesRead, aobPattern, result + aobPattern.Length);
        ...
        private int FindPattern(byte[] body, byte[] pattern, int start = 0)
        {
            int foundIndex = -1;
            ...
            for (int index = start; index <= body.Length - pattern.Length; index++)
            {
                if (body[index] == pattern[0])
                {
                    ...
                    {
                        if (body[index + index2] == pattern[index2]) continue;
                        match = false;
                        break;
        ...
        private unsafe int FindPattern(byte* body, int bodyLength, byte[] pattern, int start = 0)
        {
            int foundIndex = -1;
